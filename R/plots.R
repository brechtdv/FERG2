...
function(x, title) {
  ...
  # merge data
  FERG2:::map1$DATA <- x$DATA[match(FERG2:::map1$ISO_3_CODE, x$ISO3)]
  ...
  # define colors
  breaks <- pretty(FERG2:::map1$DATA)
  cat <- cut(FERG2:::map1$DATA, breaks)
  ...
  # setup plot
  par(mar = c(0, 0, 0, 0))
  plot(st_geometry(FERG2:::map1), col = cols, bg = col_bg, border = col_br, lwd = .75)
  plot(st_geometry(FERG2:::map3), add = T, lwd = .75,
     col = c(col_na, col_bg, col_na),
     border = c(col_br, col_lk, col_br))
  plot(st_geometry(FERG2:::map2), add = T, lwd = .75,
       col = "white", lty = 1)
  plot(st_geometry(FERG2:::map2), add = T, lwd = .75,
